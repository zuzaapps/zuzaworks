...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      program_id, intake_cohort, legal_status, application_date, start_date,
      expected_end_date, highest_qualification, qualification_institution,
      qualification_year, bank_name, account_number, account_type, branch_code,
      tax_number, data_consent, notes
    } = body;
    ...
    const program = queryOne<any>('SELECT id FROM intern_programs WHERE id = ?', [program_id]);
    if (!program) {
      return NextResponse.json(
    ...
    const result = execute(`
      INSERT INTO interns (
        ...
        disability_status, email, phone, physical_address, postal_address,
        emergency_contact_name, emergency_contact_relationship, emergency_contact_phone,
        program_id, intake_cohort, intern_status, legal_status, application_date,
        registration_date, start_date, expected_end_date, highest_qualification,
        qualification_institution, qualification_year, bank_name, account_number,
        account_type, branch_code, tax_number, data_consent, data_consent_date, notes
    ...
    `, [
      ...
      legal_status, application_date || new Date().toISOString().split('T')[0],
