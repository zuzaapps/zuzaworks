...
export async function GET(request: NextRequest) {
  ...
  try {
    let where = '1=1';
    const params: any[] = [];
    if (isActive !== null) { where += ' AND is_active = ?'; params.push(isActive); }
    ...
    return NextResponse.json({ success: false, error: 'Failed to fetch programs', message: error.message }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      qualification_id, funding_source, stipend_amount, stipend_currency,
      start_date, end_date, program_manager, training_coordinator, notes
    } = body;

    if (!program_name || !program_type) {
      return NextResponse.json({ success: false, error: 'program_name and program_type are required' }, { status: 400 });
    }
    ...
    `, [
      ...
      stipend_currency || 'ZAR', start_date || null, end_date || null,
      program_manager || null, training_coordinator || null, notes || null
    ]);
    ...
    return NextResponse.json({ success: false, error: 'Failed to create program', message: error.message }, { status: 500 });
  }
}
